    ...
    return(
        ...
        <div className="flex gap-10 bg-gray-100">
            <div className="w-200 h-200 m-10 p-6 rounded-2xl">
                <img src={car.imageURL} alt="" className="w-full object-contain rounded-2xl" />
            ...
            <div className=" flex flex-col max-w-3xl m-10 p-6 space-y-4">
                ...
                <p className="text-black text-xl font-semibold">{car?.brand}. {car?.year}</p>
                <h3 className="text-3xl font-semibold text-black">${car?.price}</h3>
                <h3 className="text-black font-semibold text-xl">Mileage: {car?.mileage} km</h3>
                <h3 className="text-black font-semibold text-xl">Color: {car?.color ?? "N/A"}</h3>
                <h3 className="text-black font-semibold text-xl">{car?.description ?? "N/A"}</h3>

                <p className="text-xl text-black">
                    Seller: {car.seller.username}
                </p>
                <div className="flex gap-10">
                    <button className="bg-blue-600 rounded-lg p-4 font-bold text-xl hover:shadow-2xl hover:bg-orange-400">Contact Seller</button>

                    <FavoriteIcon carId={car.id} initialFavorited={isFavorited} disabled={!user}/>
                </div>
